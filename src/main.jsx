...
const mortarPrices = [
  { name: 'Раствор известковый', price: 5200 },
...
function runSelfTests() {
  console.assert(formatRub(5200).replace(/\s/g, '') === '5200', 'formatRub formats 5200');
  console.assert(getDeliveryPrice(10) === 4000, 'delivery within 10 km costs 4000');
  console.assert(getDeliveryPrice(15) === 4600, 'delivery after 10 km adds 120 per km');
  console.assert(prices['М300'] * 10 === 60000, 'M300 calculation is correct');
}
    ...
    <div className="site">
      <header className="header">
        <div className="container header-inner">
          ...
          <a className="call-link" href="tel:+73473214585"><Phone size={16} /> 8 (3473) 21-45-85</a>
        </div>
      </header>
      ...
      <main>
          ...
          <div className="container hero-grid">
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <div className="hero-actions">
                ...
                <Button variant="outline"><Send size={18} /> Написать в Telegram</Button>
              </div>
              <div className="stats">
        ...
        <section id="calculator" className="container section calc-grid">
          <div>
            ...
            <p className="section-note">Введите объём и выберите марку. После заявки менеджер проверит расчёт, доставку и подскажет оптимальное решение.</p>
            <div className="check-list">
              {['Подскажем, какая марка нужна под ваш объект', 'Рассчитаем доставку и график подачи миксера', 'При необходимости организуем дополнительные услуги'].map((item) => (
          ...
          <Card className="form-card">
            ...
            <Button variant="light">Зафиксировать расчёт</Button>
          </Card>
        </section>
        ...
        <section id="fleet" className="section section-dark">
          <div className="container">
            <div className="section-head">
              ...
              <p>Используем реальные фото завода и миксеров. Для сайта они оформлены в более чистом, премиальном стиле.</p>
            </div>
            <div className="fleet-grid">
              {fleetPhotos.map((item, index) => (
                <motion.div key={item.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.55, delay: index * 0.12 }} className="fleet-card">
                  <div className="fleet-img"><img src={item.image} alt={item.title} /><span><Camera size={16} /> Фото объекта</span></div>
                  <div className="fleet-body"><h3>{item.title}</h3><p>{item.text}</p></div>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <section className="section section-dark">
          <div className="container">
            <div className="center-head"><p className="eyebrow">Отзывы</p><h2>Что говорят клиенты</h2></div>
            <div className="reviews-grid">
              {reviews.map((review, index) => (
                <motion.div key={review.name} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45, delay: index * 0.1 }}>
        ...
        <section className="container section map-grid">
          <div>
            ...
            <p className="section-note">Республика Башкортостан, г. Стерлитамак. Карта поможет быстро построить маршрут до производства или рассчитать доставку до объекта.</p>
            <div className="map-note"><Award size={32} /> Здесь можно вставить iframe-код Яндекс.Карты из кабинета Яндекс.Бизнес или конструктора карт.</div>
          </div>
          <div className="map-placeholder"><MapPin size={54} /><strong>Карта Яндекс</strong><span>Место для интерактивной карты</span></div>
        </section>
        ...
        <section id="delivery" className="section section-dark">
          <div className="container">
            ...
            <div className="steps">
              ...
              ].map(([num, title, text]) => <div key={num} className="step"><strong>{num}</strong><h3>{title}</h3><p>{text}</p></div>)}
            </div>
          </div>
        </section>
        ...
        <section className="container section">
          <div className="cta">
            <div><h2>Нужен бетон в Стерлитамаке?</h2><p>Оставьте заявку — быстро рассчитаем стоимость, доставку и ближайшее время подачи миксера.</p></div>
            <div className="hero-actions"><Button variant="dark"><Phone size={18} /> Позвонить</Button><Button variant="outline-dark"><MessageCircle size={18} /> Telegram</Button></div>
          </div>
        </section>
      </main>
      ...
      <footer id="contacts" className="footer">
        <div className="container footer-grid">
          <div><p className="brand-title">Бетонсервис</p><p>Продажа бетона и раствора с доставкой на объект.</p></div>
          <div><p>Телефон: 8 (3473) 21-45-85</p><p>Telegram: +7 (917) 419-48-25</p><p>Email: bsk04@mail.ru</p></div>
          <div><p>Стерлитамак, Республика Башкортостан</p><p>Заявки принимаем ежедневно</p></div>
        </div>
      </footer>
    </div>
  );
